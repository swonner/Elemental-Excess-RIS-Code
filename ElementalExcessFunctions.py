...
def setup(ANALYSISPATH):
    ...
    if not os.path.isdir(ANALYSISPATH + "/ELEMENTALEXCESSDATA"):
        os.mkdir(ANALYSISPATH + "/ELEMENTALEXCESSDATA")
        
    if not os.path.isdir(ANALYSISPATH + "/ELEMENTALEXCESSMINMAXDATA"):
        os.mkdir(ANALYSISPATH + "/ELEMENTALEXCESSMINMAXDATA")
        
    if not os.path.isdir(ANALYSISPATH + "/ELEMENTALEXCESSPLOTS"):
        os.mkdir(ANALYSISPATH + "/ELEMENTALEXCESSPLOTS")
...
def integratingthedata(ANALYSISPATH, filename, linenumber, element, matrixshifted_minusbulkavg, expectedminormax, linescanhalfdistance, integratecutoff, plotcolor, viewplots):
    ...
    ax.tick_params(which='major', length=8)
    ax.tick_params(which='minor', length=4)
    figurestring = ANALYSISPATH + "/ELEMENTALEXCESSPLOTS/" +  filename + "_line" + str(linenumber) + "-" + element + '.png'
    plt.savefig(figurestring, bbox_inches='tight')
    if viewplots == "no":
        plt.close()
    ...
    data.to_csv(ANALYSISPATH + "/ELEMENTALEXCESSDATA/" + filename + "_line" + str(linenumber) + "-" + element + "_ELEMENTALEXCESSData" + ".txt", index=False,)
    ...
    data2.to_csv(ANALYSISPATH + "/ELEMENTALEXCESSMINMAXDATA/" + filename + "_line" + str(linenumber) + "-" + element + "_ELEMENTALEXCESSMinMaxData" + ".txt", index=False,)
    return 
