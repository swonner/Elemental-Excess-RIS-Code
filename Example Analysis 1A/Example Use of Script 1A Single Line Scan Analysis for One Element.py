...
'''
This script can be utilized to calculate the elemental excess for one line scan for one element. 
By default this script centers the line scan on the main element being analyzed. 
To center the line scan using an element different than the analyzed line scan, see Script 1B. 


The assumed line scan input structure is a text file with two columns. 
...
datafolder = "Line Scan"                           # ENTER THE DIRECTORY PATH WHERE THE LINE SCAN DATA IS BEING STORED
...
## Element Info ##
mainelement = "Cr"                          # ENTER THE ELEMENT BEING INVESTIGATED 
expectedminormax = "min"                     # ENTER WHETHER THE MAIN ANALYZED ELEMENT IS EXPECTED TO HAVE A LOCAL MIN OR MAX AT THE GB. options: "min" and "max"
...
plotcolor = "#44AA99"                           # ENTER THE COLOR YOU WANT THE PLOT TO BE


## Adjusting the run ##
viewplots = "yes"                          # ENTER "yes" IF YOU WANT TO SEE THE PLOTS INTERACTIVELY; ENTER "no" IF YOU DO NOT. OPTING YES COULD BE MEMORY INTENSIVE.



'''
OPTION 1A: A SINGLE ELEMENT, A SINGLE LINE SCAN
Can be used to process a single elemental line scan. 
The grain boundary will be identified and centered by the local min/max concentration of that specific element. 
'''

ElementalExcessFunctions.analysis_singlelinescan(ANALYSISPATH, datafolder, filename, linenumber, mainelement, expectedminormax, centerelement, centerelementexpectedminormax,  plotcolor,  atomicvolume, L_guess_nm_alongline, weight, fraction, linescanhalfdistance, integratecutoff, viewplots)
